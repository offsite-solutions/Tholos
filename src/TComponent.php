<?php /** @noinspection DuplicatedCode SpellCheckingInspection PhpUnusedFunctionInspection NotOptimalIfConditionsInspection */
  ...
  class TComponent {
     ...
     * @see getProperty
     */
    private function parsePropertyValue2($property_value_, $notFound_ = false) {
     ...
     * @return array|mixed Property data or $notFound_ value when property is not found
     */
    public function getProperty(string $name_, $notFound_ = false) {
      ...
      if (isset($this->_properties[$propName])) {
        ...
        if (!is_array($prop_value)) {
          ...
          if ($returnValue === $prop_value && strpos($prop_value, '@') === false) {
     ...
     */
    public function getPropertyType(string $name_, $notFound_ = false) {
     ...
     */
    public function getPropertyComponentId(string $name_, $notFound_ = false) {
     ...
     * @param mixed $notFound_ Value to return when name is not found. Defaults to false.
     * @return array|mixed Event data or $notFound_ value when event is not found
     */
    public function getEvent(string $name_, $notFound_ = false): array {
      ...
      foreach ($this->_events as $key => $event) {
        ...
        if ($event['type'] === 'GUI') {
          if ($value) { // az ertek van megadva
            ...
            $genparameters = "'" . $this->getProperty('ID', '') . "'," .
            ...
            if (strpos($value, '(') >= 0) {
              if (strpos($value, ')') > strpos($value, '(') + 1) { // a user ilyet adott meg jsfunc(valami)
                $jsevent = Eisodos::$utils->replace_all($value, '(', '(' . $genparameters . ',', false, false);
              } else { // a user ilyet adott meg jsfunc()
                $jsevent = Eisodos::$utils->replace_all($value, '(', '(' . $genparameters, false, false);
              }
            } else { //
              $jsevent = $value . '(' . $genparameters . ');';
            }
