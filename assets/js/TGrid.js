...
function TGrid_setScrollable(formId_, scrollable_) {                     // scrollable
    $("#helper_" + formId_ + " #TGrid_Scrollable_").val(scrollable_);
    Tholos.eventHandler(formId_,formId_,'TGrid','refresh');
}

function TGrid_setTransposed(formId_, transposed_) {                     // scrollable
...
function TGrid_showFilterParameters(modalId_, filterDatatype_, filterListSource_, filterListSourceRoute_, filterListFilter_, filterFieldId_, filterFieldText_) {
    ...
    $(modalId_+" .TGrid-filter-" + filterDatatype_).trigger("TGrid.show", {filterListSource: filterListSource_, filterListSourceRoute: filterListSourceRoute_,filterListFilter: filterListFilter_, filterFieldId: filterFieldId_, filterFieldText: filterFieldText_});   // udatatable show esemeny letrehozasa
}
...
function TGrid_regenerateTextFilters(formId_, filterSlot_) {
    ...
    for (var i = 1; i < 100; i++) {
        if ((filterSlot_ > 0 && i == filterSlot_) || (filterSlot_ < 1 && $("#helper_" + formId_ + " #" + Tholos.getComponentName(formId_) + "_f_" + i).length > 0)) {                               // ha van ilyen filter, akkor
            var filterParams = TGrid_explode(":", $("#helper_" + formId_ + " #" + Tholos.getComponentName(formId_) + "_f_" + i).val(), 3);
            var filters = eval("TGrid_filters_" + formId_);
            for (var j = 0; j < filters.length; j++) if (filters[j].id == filterParams[0]) var filter = filters[j];

            var textFilter = '<span class="label label-info" style="margin-bottom: 3px;" id="filterslot' + i + '">' +  // filterslot
    ...
    fc.find(".setbutton").on("click", function (event) {                     // mentes gomb esemenye
        ...
        if (slotId_ > 0) {                                                           // ha szerkesztes, akkor
            ...
            $("#helper_" + formId_).append($('<input>').attr({                           // input hidden mezo letrehozasa
                type: 'hidden', id: Tholos.getComponentName(formId_) + '_f_' + filterSlot, name: Tholos.getComponentName(formId_) + '_f_' + filterSlot,
                value: filterValue
            }));
        }
        $("#header_" + formId_).show();
